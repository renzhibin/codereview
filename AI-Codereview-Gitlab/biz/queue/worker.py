...
from biz.utils.log import set_trace_id
    ...
    try:
        ...
        if push_review_enabled:
            ...
            if len(changes) > 0:
                ...
                changed_files = ', '.join(item.get('new_path', '') for item in changes)
                repo_url = webhook_data['project'].get('git_http_url') or webhook_data.get('repository', {}).get('http_url_to_repo')
...
def handle_merge_request_event(webhook_data: dict, gitlab_token: str, gitlab_url: str, gitlab_url_slug: str, trace_id: str):
    '''
    ...
    :param trace_id: 追踪ID，用于日志追踪
    :return:
    '''
    # 设置 traceid
    set_trace_id(trace_id)
    logger.info(f'Merge Request Hook event received, trace_id: {trace_id}')
    ...
    try:
        ...
        # review 代码（传递仓库信息以支持上下文获取）
        commits_text = ';'.join(commit['title'] for commit in commits)
        changed_files = ', '.join(item.get('new_path', '') for item in changes)
        repo_url = webhook_data['project'].get('git_http_url') or webhook_data['project'].get('http_url_to_repo')
    ...
    try:
        ...
        if push_review_enabled:
            ...
            if len(changes) > 0:
                commits_text = ';'.join(commit.get('message', '').strip() for commit in commits)
                changed_files = ', '.join(item.get('new_path', '') for item in changes)
                repo_url = webhook_data['repository'].get('clone_url') or webhook_data['repository'].get('html_url')
