...
def handle_queue(function: callable, data: any, token: str, url: str, url_slug: str, trace_id: str = None):
    if queue_driver == 'rq':
        ...
        if trace_id:
            queues[url_slug].enqueue(function, data, token, url, url_slug, trace_id)
        else:
            queues[url_slug].enqueue(function, data, token, url, url_slug)
    else:
        if trace_id:
            process = Process(target=function, args=(data, token, url, url_slug, trace_id))
        else:
