...
from biz.utils.log import generate_trace_id_from_mr
...
def handle_gitlab_webhook(data):
    ...
    if object_kind == "merge_request":
        # 生成 traceid（基于 MR URL + IID）
        trace_id = generate_trace_id_from_mr(data)
        logger.info(f'GitLab MR event, trace_id: {trace_id}')
        # 创建一个新进程进行异步处理
        handle_queue(handle_merge_request_event, data, gitlab_token, gitlab_url, gitlab_url_slug, trace_id)
        # 立马返回响应
        return jsonify(
            {'message': f'Request received(object_kind={object_kind}), will process asynchronously.', 'trace_id': trace_id}), 200
